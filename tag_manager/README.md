...
确保机器已安装 Python 3.11+，然后在仓库根目录双击 `start.bat`：首次运行会创建虚拟环境并安装依赖，随后启动服务。
...
```powershell
$env:WARDROBE_VIDEO_DECRYPTOR_ROOT = "C:\path\to\comfyui-encrypt-video"
python -m tag_manager.run
```
...
上游以 AGPL-3.0 发布；衣柜采用外部适配模式，不复制其源码。
...
完整字段与调用方式见仓库根目录 README 的「开放 API」小节。
