...
- **视频解密** — 流式上传插件 2.0 生成的 `.evideo`，由单任务后台队列认证解密并无重编码整理为带完整播放索引的 H.264/AAC MP4
...
- **LLM 助手** — 接入大模型 API；配置入口在工坊齿轮弹层，Copilot 对话承担辅助分类与提示词建议
- **AI 提示词抽卡** — 共用服务端 LLM 配置，将抽卡数据保存到 SQLite
- **魔导书导入** — 从 `提示词/魔导书.xlsx` 批量导入 tag，自动识别分类和二级分类

## 快速部署（Windows）

确保机器已安装 Python 3.11+，然后：

1. 双击 `setup_venv.bat` — 自动创建虚拟环境并安装依赖（仅首次）
2. 双击 `start.bat` — 启动服务
...
视频解密复用独立的 `comfyui-encrypt-video` 上游仓库。通过环境变量指定其根目录：

```powershell
$env:WARDROBE_VIDEO_DECRYPTOR_ROOT = "D:\User\Github\comfyui-encrypt-video"
python -m tag_manager.run
```

也可以在本地 `tag_manager/video_decrypt_config.json` 中配置 `upstream_root`；该文件受 `.gitignore` 忽略。进入侧边栏“视频解密”，选择插件 2.0 生成的 `.evideo`、输入密码并提交，完成后下载带普通索引、可显示总时长和拖动进度的 MP4。

- 上传按 1 MiB 分块写入，不会一次性把完整视频读入内存。
- 同一时间只处理一个视频，其他任务依次排队。
- 密码只在当前请求和任务内存中使用，不写入数据库或任务历史。
- 应用重启时未完成任务会标记为“已中断”，需要重新提交密码。
- `.evideo` 使用 Scrypt 派生密钥与 AES-256-GCM 认证加密，密码错误或密文被篡改都会终止并清理部分输出。
- 解密直接恢复上游压缩好的 H.264/YUV420P + AAC MP4，不会再次转码或损失画质。
- 2.0 不兼容 1.x 的像素置换 MKV；旧文件应使用保留的 1.4.0 上游工具先恢复。

上游以 AGPL-3.0 发布；衣柜采用外部适配模式，不复制其源码。详细迁移记录见本地共同文档中的 v1.4.0 实施记录。
