...
class WorkshopWorkbenchLayoutTests(unittest.TestCase):
    ...
    def test紧凑Header与操作区(self) -> None:
        ...
        self.assertIn('class="copilot-settings-drawer" id="wsLlmSettingsDialog"', tpl)
        self.assertIn("/api/copilot/settings", tpl)
        self.assertIn("/static/copilot-settings.js?v=5", tpl)
...
class LlmSettingsEmbedTests(unittest.TestCase):
    ...
    def testOpenAPI版本与v1操作数(self) -> None:
        ...
        self.assertEqual("1.24.12", app_module.app.version)
        self.assertEqual("1.24.12", schema["info"]["version"])
